...
パスはパンくずとして使える。**階層名をクリックするとその階層へ移動する**ので、
深い場所から数段上へ戻るのに Left を繰り返さなくてよい。今いるフォルダ名は白の太字、
その手前の階層は灰色で表示する。幅に収まらず削られた部分はクリックできない。

画面の上枠にモード名、その下の行にパスを置く。モードは変わらない操作子なので枠に、
パスは移動のたびに変わるので専用の行に置いている。どちらもクリックできる。

画面は端末の全高を使う。上に残っていたシェルの履歴はスクロールして退避し、終了すると元の表示に戻る。
履歴はスクロールバックに残る。
...
画面は変化があったときだけ描き直す。走査中は件数とスピナーが動くので毎フレーム、
それ以外は入力があったときだけになる。開いたまま放置しても CPU を使い続けない。
