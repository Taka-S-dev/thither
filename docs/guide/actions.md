...
行の左に出ているキーを押すと、選択や絞り込みをせずにその操作を実行する。
絞り込みで隠れている操作にも効くので、押すキーは一覧の中身に左右されない。
...
一時コピーのフォルダを開く操作にはキーがない。繰り返す操作ではないので、
文字を利用者のために空けてある。
...
| `key` | `alt+g` の形で 1 文字。省略すると割り当てない |
| `include_defaults` | `false` にすると自作アクションだけを表示。既定は `true` |

`key` は上の表の組み込みと同じ文字も指定できる。その場合は自作の操作が優先し、
