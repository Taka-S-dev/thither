...
本体は UTF-8 でパスを書くので、日本語を含むパスは cp932 のままでは化ける。
cmd のシムは `chcp 65001` してから本体を呼び、終わったら元のコードページに戻す。
PowerShell のシムも同じ理由で `[Console]::OutputEncoding` を呼び出しの間だけ UTF-8 にする。
