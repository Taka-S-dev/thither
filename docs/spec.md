...
3 列は同じ重みでは出さない。真ん中が主役なので幅を広く(20% / 45% / 残り)取り、
両脇は文脈として灰色に落とす(ファイル 244、フォルダ 67)。真ん中のフォルダは 75 の明るい青。
どこにいるかを追えるよう、ヘッダのパスは末尾のフォルダ名だけ太字にし、親列では今いるフォルダを反転する。
列に収まらない名前は末尾を `…` にする(全角は 2 桁として数える)。
